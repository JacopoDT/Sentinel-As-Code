...
[`Scripts/Deploy-CustomContent.ps1`](../../Deploy/Deploy-CustomContent.ps1).

| Concern | Where |
| --- | --- |
| Rule files | [`AnalyticalRules/`](./) |
| Deploy logic | [`Scripts/Deploy-CustomContent.ps1`](../../Deploy/Deploy-CustomContent.ps1) (function `Deploy-CustomDetections`, ~line 1077) |
| Drift detection | See [Sentinel Drift Detection](../../Deploy/Sentinel-Drift-Detection.md) |
...
[`AnalyticalRules/`](./). Add new ones as needed; the deploy
...
- **`triggerOperator`** must use short form (`gt`, `lt`, `eq`, `ne`). The
  ...
  — see the table at [`Deploy-CustomContent.ps1:1185-1192`](../../Deploy/Deploy-CustomContent.ps1).
...
- **`enabled`** — boolean. `true` means the rule runs immediately after
  ...
  | Case | Reference |
  | --- | --- |
  | Rule lives under `AnalyticalRules/Community/**` | [`Deploy-CustomContent.ps1:1155`](../../Deploy/Deploy-CustomContent.ps1) |
  | A required data type / watchlist / parser dependency is missing at deploy time | [`Deploy-CustomContent.ps1:1146`](../../Deploy/Deploy-CustomContent.ps1) |
  | KQL validation fails at deploy (e.g. a freshly deployed watchlist isn't queryable yet) | [`Deploy-CustomContent.ps1:1259`](../../Deploy/Deploy-CustomContent.ps1) |

  Because `enabled` is routinely overridden at deploy time, the [drift
  detector](../../Deploy/Sentinel-Drift-Detection.md) deliberately excludes it from
...
- **Deprecated rules** — rules with `[Deprecated]` in the display name are
  ...
  ([`Deploy-SentinelContentHub.ps1:1153-1157`](../../Deploy/Deploy-SentinelContentHub.ps1)).
...
| `version` | string | Semver (e.g., `1.0.0`). The drift sync bumps the patch component when it absorbs portal edits — see [Sentinel Drift Detection](../../Deploy/Sentinel-Drift-Detection.md#how-custom-drift-gets-absorbed). |
...
The deploy logic lives in [`Scripts/Deploy-CustomContent.ps1`](../../Deploy/Deploy-CustomContent.ps1)
(function `Deploy-CustomDetections`). Notable behaviours that affect how
you should author rules:

| Behaviour | Reference |
| --- | --- |
| Rules deploy `enabled: true` by default. Override with `enabled: false` in the YAML. | [`Deploy-CustomContent.ps1:1155`](../../Deploy/Deploy-CustomContent.ps1) |
| Rules under `AnalyticalRules/Community/**` always deploy disabled. | [`Deploy-CustomContent.ps1:1155`](../../Deploy/Deploy-CustomContent.ps1) — see [Community Rules](Community-Rules.md) |
| If `requiredDataConnectors` reference data types that aren't present yet, the rule deploys disabled and waits. | [`Deploy-CustomContent.ps1:1146`](../../Deploy/Deploy-CustomContent.ps1) |
| If KQL validation fails at deploy time (e.g. a freshly deployed watchlist column isn't queryable yet), the rule retries deployment with `enabled: false`. | [`Deploy-CustomContent.ps1:1259`](../../Deploy/Deploy-CustomContent.ps1) |
| Smart-deploy mode (`-SmartDeployment`) only redeploys files changed since the last successful run. Bumping `version` is not required but the drift sync bumps it automatically when absorbing portal edits. | [`Deploy-CustomContent.ps1:292`](../../Deploy/Deploy-CustomContent.ps1) |

## Related docs

- [Sentinel Drift Detection](../../Deploy/Sentinel-Drift-Detection.md) — daily detection of
