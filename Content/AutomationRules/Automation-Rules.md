...
Source files live under [`AutomationRules/`](./).
...
The deployment script ([`Scripts/Deploy-CustomContent.ps1`](../../Deploy/Deploy-CustomContent.ps1)) discovers all `*.json` files recursively under this directory and deploys each one via the REST API.
...
See [`AutomationRules/AutoCloseInformational.json`](./AutoCloseInformational.json).

### Add an investigation task to high severity incidents

See [`AutomationRules/AddTaskOnHighSeverity.json`](./AddTaskOnHighSeverity.json).
...
5. Restructure the output into the schema above (top-level `automationRuleId`, `displayName`, `order`, `triggeringLogic`, `actions`) and save as a `.json` file in [`AutomationRules/`](./).
...
These roles should be assigned to the service principal or managed identity configured in the pipeline. See [Pipelines](../../Pipelines/Pipelines.md) for pipeline configuration details and [Scripts](../../Deploy/Scripts.md#setup-serviceprincipalps1) for the bootstrap script.
